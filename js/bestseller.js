...
// Manejar clic en el botón "Ver más" para la sección de reseñas
$(document).on("click", ".ver-mas-btn-titulo", function () {
    let titulo = $(this).data("titulo");
    cargarMasLibrosPorTituloReseñas(titulo);
});
...
// Función para cargar libros desde la API por título en la sección de reseñas
function cargarLibrosPorTituloReseñas(titulo) {
    if (!titulo) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (let i = 0; i < Math.min(4, response.items.length); i++) {
                agregarLibroPorCategoria(response.items[i].volumeInfo, "reseñas");
            }

            if (response.items.length > 4) {
                $("#reseñas-container").append(`<button class="btn btn-primary ver-mas-btn-titulo" data-titulo="${titulo}">Ver más</button>`);
            }
        },
        error: function (xhr, status, error) {
            console.error("Error al cargar los libros:", error);
...
// Función para cargar más libros por título en la sección de reseñas
function cargarMasLibrosPorTituloReseñas(titulo) {
    let apiUrl = "https://www.googleapis.com/books/v1/volumes?q=" + titulo;
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $(".ver-mas-btn-titulo[data-titulo='" + titulo + "']").remove();
